...
@pytest.mark.django_db
def test_official_diplomas_tex_smoke(client):
    # 2-digit day and month to avoid bothering with zero padding
    date1 = date(2020, 12, 20)
    date2 = date(2020, 11, 15)
    g1, g2 = GraduateProfileFactory.create_batch(2, diploma_issued_on=date1)
    ...
    assert response.status_code == 200
    assert smart_bytes(g1.student_profile.user.last_name) in response.content
    assert smart_bytes(g2.student_profile.user.last_name) in response.content
    assert smart_bytes(g3.student_profile.user.last_name) not in response.content