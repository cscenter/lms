...
class TestElectronicDiplomaExportService:
    ...
    def test_get_meta_courses_data(self):
        ...
        assert self.meta_course1.name in meta_courses
        assert self.meta_course2.name in meta_courses
        assert meta_courses[self.meta_course1.name] == self.meta_course1.index
        assert meta_courses[self.meta_course2.name] == self.meta_course2.index
    ...
    def test_with_shad_courses(self):
        """Test the service with SHAD course records."""
        # Create SHAD course record for the first student
        shad_course = SHADCourseRecordFactory(
            student=self.student_profile1.user,
            name="SHAD Course",
            grade=GradeTypes.EXCELLENT,
            semester=self.semester
        )
        ...
        for profile in student_profiles:
            if profile.id == self.student_profile1.id:
                profile.user.prefetched_enrollments = [
                    self.enrollment1, self.enrollment2
                ]
                # Convert birth_date string to datetime object if it's a string
                import datetime
                if isinstance(profile.user.birth_date, str):
                    profile.user.birth_date = datetime.datetime.strptime(profile.user.birth_date, '%Y-%m-%d').date()
            elif profile.id == self.student_profile2.id:
                profile.user.prefetched_enrollments = [
                    self.enrollment3
                ]
                # Convert birth_date string to datetime object if it's a string
        ...
        # Check that the SHAD course is included in the control_mc_sum
        # The first student should have 3 courses (2 regular + 1 SHAD)
        assert int(rows[1][13]) == 3
        
        # The second student should have 1 course
        assert int(rows[2][13]) == 1
