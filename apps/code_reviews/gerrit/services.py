...
from code_reviews.gerrit.ldap_service import (
    connect_gerrit_auth_provider, get_ldap_username, update_ldap_user_password_hash
)
...
def get_branch_name(student_profile: StudentProfile, course: Course) -> str:
    ...
    # TODO: Always attach branch code. Remove this `if` statement in spring 2022
...
def add_assignment_comment_about_new_change(student_assignment: StudentAssignment,
    ...
    # TODO: replace with create_assignment_comment
...
def create_change(client, student_assignment: StudentAssignment) -> Optional[GerritChange]:
    ...
    enrollment = (Enrollment.objects
                  ...
                  .select_related('student_profile__branch')
                  .first())
    if not enrollment:
...
def normalize_code_review_score(value: int, assignment: Assignment) -> Decimal:
    """
    Converts code review score to the value suited for LMS.

    Gerrit uses 5-point score system: -2, -1, 0, +1, +2
        [-2, -1, 0] - zero score in LMS
        +1 - half of the maximum score in LMS
        +2 - maximum score in LMS
    """
    score_max = Decimal(assignment.maximum_score)
    if value >= 2:
        normalized_value = score_max
    elif value == 1:
        normalized_value = score_max / 2
    else:
