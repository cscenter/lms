...
def test_info_blocks_manager_for_site(settings):
    u1, u2 = InfoBlockFactory.create_batch(2, site__domain=TEST_DOMAIN)
    u3 = InfoBlockFactory(site__domain=ANOTHER_DOMAIN)

    infoblocks_test = list(InfoBlock.objects.for_site(TEST_DOMAIN_ID))
