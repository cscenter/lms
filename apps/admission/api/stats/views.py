...
from .pandas_serializers import (
    ApplicationSubmissionPandasSerializer,
    CampaignResultsByEducationLevelSerializer,
    CampaignResultsByUniversitiesSerializer,
    CampaignResultsTimelineSerializer,
    ScoreByCoursesSerializer,
    ScoreByUniversitiesSerializer,
)
...
TestingCountAnnotation = Count(
    Case(
        When(Q(online_test__isnull=False, online_test__score__isnull=False), then=1),
        output_field=IntegerField(),
    )
)
ExaminationCountAnnotation = Count(
    Case(When(exam__isnull=False, then=1), output_field=IntegerField())
)
InterviewsCountAnnotation = Count(
    Case(When(interviews__isnull=False, then=1), output_field=IntegerField())
)
...
class CampaignStagesByYears(ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        applicants = (
            Applicant.objects.filter(campaign__branch=branch_id)
            .values("campaign_id", "campaign__year")
            .annotate(
                application_form=Count("pk"),
...
class CampaignStagesByUniversities(ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            Applicant.objects.filter(campaign_id=campaign_id)
            .values("university_legacy_id", "university_legacy__name")
            .annotate(
                application_form=Count("campaign_id"),
                testing=TestingCountAnnotation,
                examination=ExaminationCountAnnotation,
                interviewing=InterviewsCountAnnotation,
            )
...
class ApplicationFormSubmissionByDays(ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        campaigns = Campaign.objects.filter(
            branch_id=self.kwargs["branch_id"], year__gte=2017
        )
    ...
    def get_filters(campaigns):
        ...
        campaign_ids = []
        for c in campaigns:
            application_period = Q(
                created__gte=c.application_starts_at, created__lte=c.application_ends_at
            )
    ...
    def get_stat(self, campaigns):
        ...
        qs = (
            Applicant.objects.filter(filters)
        ...
        for created, total in qs:
            ...
            if index >= len(data[created.year]):
                ...
                logger.error(
                    f"Wrong index {index} for {created}. "
...
class CampaignStatsApplicantsResults(ListRenderersMixin, PandasView):
    ...
    def get_queryset(self):
        branch_id = self.kwargs.get("branch_id")
        qs = (
            Applicant.objects.filter(campaign__branch_id=branch_id, status__in=STATUSES)
...
class CampaignResultsByUniversities(ListRenderersMixin, PandasView):
    ...
    def get_queryset(self):
        campaign_id = self.kwargs.get("campaign_id")
        qs = (
            Applicant.objects.filter(campaign_id=campaign_id, status__in=STATUSES)
...
class CampaignStatsTestingScoreByUniversities(ListRenderersMixin, PandasView):
    ...
    def get_queryset(self):
        campaign_id = self.kwargs.get("campaign_id")
        return (
            Test.objects.filter(applicant__campaign_id=campaign_id)
...
class CampaignStatsExamScoreByUniversities(ListRenderersMixin, PandasView):
    ...
    def get_queryset(self):
        campaign_id = self.kwargs.get("campaign_id")
        return (
            Exam.objects.filter(applicant__campaign_id=campaign_id)
            .values("score", "applicant__university_legacy__name")
            .annotate(total=Count("score"))
        )
