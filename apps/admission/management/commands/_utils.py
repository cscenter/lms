...
APPROVAL_DIALOG = (
    "There is no undo. Only 'y' will be accepted to confirm.\n\nEnter a value: "
)
...
def validate_campaign_passing_score(campaign):
    if not campaign.online_test_passing_score:
        raise ValidationError(
            f"Passing score for campaign '{campaign}' must be non zero."
        )
...
def validate_campaign_contests(campaign, contest_type):
    ...
    qs = Contest.objects.filter(type=contest_type, campaign=campaign)
    if not qs.exists():
        raise ValidationError(
            f"Contests of type {contest_type} not found for campaign {campaign}"
        )
...
class CurrentCampaignMixin(BaseCommandMixin):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--branch", type=str, help="Branch code to restrict current campaigns"
        )

    def get_current_campaigns(
        self, options, confirm=True, branch_is_required=False
    ) -> List[Campaign]:
        ...
        campaigns = list(
            Campaign.objects.select_related("branch").filter(*filter_params)
        )
        self.stdout.write(f"Selected campaigns ({len(campaigns)} total):")
        for campaign in campaigns:
            self.stdout.write(
                f"\t[{campaign.branch.site}] {campaign.branch.name}, {campaign.year}"
            )
        self.stdout.write("")
...
class EmailTemplateMixin(BaseCommandMixin):
    ...
    def validate_templates(
        self, campaigns, template_name_patterns: Iterable[str], confirm=True
    ):
        """
    ...
    def get_template_name(
        self, campaign: Campaign, pattern: Optional[str] = None, **kwargs
    ) -> str:
        pattern = pattern or self.TEMPLATE_PATTERN
        return pattern.format(
            year=campaign.year, branch_code=campaign.branch.code, **kwargs
        )
