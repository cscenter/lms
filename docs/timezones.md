...
Сейчас есть проблема при следующей последовательности действий. пускай datetime зависит от поля `city`:
    * Обновляем `city`
    * Для текущей модели меняется значение таймзоны и мы должны пересчитать время с учетом нового часового пояса, но этого не происходит, т.к. 
    в `cleaned_data` старый часовой пояс, даже если изменения попали в `changed_data`
